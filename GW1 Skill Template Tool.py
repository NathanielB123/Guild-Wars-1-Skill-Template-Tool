...
def DrawMonster():
    print("Here you can draw random GW1 enemys")
...
so it should be quite simple to look them up and then create the skill template using this tool.""")
    ...
    while True:
        ...
        if Input in ("Draw", "D"):
            ...
            Choice = None
            while Choice is None or StoredNPCs[Choice][CAMPAIGN] != TempCampaign or (not Bosses and StoredNPCs[Choice][TYPE]==BOSS) or (not Allies and StoredNPCs[Choice][TYPE]==ALLY):
...
def Add(NewData, Data):
    ...
    for Line in NewData:
        ...
        if Line=="Allies" or Line == "Foes" or Line=="Bosses":
            ...
            Split = Line.split(" ")
            if Split[0] in ProfessionDict.keys() or Split[0] in DualProfessions:
                if "(" in Line:
                    Name = Line[Line.rfind(")")+2::]
                    if Name[-1].isdigit():
                        Name = Name[0:-1]
                    #Names ending in brackets will be empty strings
                    if Name not in Data.keys() and not Name=="":
                        Class = Split[0]
                        i=1
                        while True:
                            Level = Split[i]
